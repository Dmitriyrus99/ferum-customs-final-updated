...
def validate(doc: "service_request", method: str | None = None) -> None:
    ...
    if doc.project and not doc.customer:
        customer = frappe.db.get_value("service_project", doc.project, "customer") # Assuming ServiceProject has a customer link
        if customer:
...
def prevent_deletion_with_links(doc: "service_request", method: str | None = None) -> None:
    ...
    if doc.name: # Ensure doc.name exists
        linked_reports = frappe.db.exists("service_report", {"service_request": doc.name})
        if linked_reports:
            frappe.throw(
...
def get_engineers_for_object(service_object: str) -> list[str]:
    ...
    service_object_doc = frappe.get_doc("service_object", service_object)
    engineers = []
